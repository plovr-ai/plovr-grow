 ...
 */
function generateOAuthUrl(tenantId: string, redirectUri: string): string {
  const state = Buffer.from(JSON.stringify({ tenantId })).toString("base64url");
  return stripeService.generateConnectOAuthUrl(getClientId(), redirectUri, state);
}
 ...
 */
async function handleOAuthCallback(code: string, tenantId: string): Promise<OAuthCallbackResult> {
 ...
 */
async function getConnectAccount(tenantId: string) {
 ...
 */
async function isAccountReady(tenantId: string): Promise<boolean> {
 ...
 */
async function disconnectAccount(tenantId: string): Promise<void> {
  const account = await stripeConnectAccountRepository.getByTenantId(tenantId);
  if (!account) {
 ...
 */
async function handleAccountUpdated(stripeAccountId: string, status: ConnectAccountStatus): Promise<void> {
