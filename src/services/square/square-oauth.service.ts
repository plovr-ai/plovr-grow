...
async function exchangeCode(code: string): Promise<SquareTokenResponse> {
...
async function refreshToken(refreshTokenValue: string): Promise<SquareTokenResponse> {
  const client = getClient();
  const tokenResponse = await client.oAuth.obtainToken({
    clientId: squareConfig.appId,
    clientSecret: squareConfig.appSecret,
...
async function listLocations(accessToken: string): Promise<SquareLocation[]> {
...
export const squareOAuthService = {
