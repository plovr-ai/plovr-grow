 ...
 */
async function getMerchantBySlugWithTenant(
  slug: string
): Promise<MerchantWithTenant | null> {
 ...
 * @param filter - 过滤条件
 */
async function getMerchantsByTenantId(
  tenantId: string,
...
async function updateMerchant(
  ...
  // 验证权限
  const existing = await getMerchant(tenantId, merchantId);
  if (!existing) {
 ...
 * @param merchantId - 商户 ID
 */
async function deleteMerchant(tenantId: string, merchantId: string): Promise<void> {
  // 验证权限
  const existing = await getMerchant(tenantId, merchantId);
  if (!existing) {
    throw new AppError(ErrorCodes.MERCHANT_NOT_FOUND, undefined, 404);
 ...
 * 检查 Merchant 是否营业中
 */
async function isOpen(merchantId: string): Promise<boolean> {
 ...
 */
async function lookupByAiPhone(phone: string) {
