...
describe("POST /api/generator/create", () => {
  ...
  it("returns existingSlug when placeId already generated", async () => {
    ...
    const res = await POST(makeRequest({ placeId: "ChIJ_test", placeName: "Joe's Pizza" }), { params: Promise.resolve({}) });
  ...
  it("returns generationId and triggers async generation", async () => {
    ...
    const res = await POST(makeRequest({ placeId: "ChIJ_new", placeName: "New Place" }), { params: Promise.resolve({}) });
    const data = await res.json();
  ...
  it("returns 400 for missing placeId", async () => {
    const res = await POST(makeRequest({ placeName: "No Place ID" }), { params: Promise.resolve({}) });
    expect(res.status).toBe(400);
  });

  it("returns 400 for missing placeName", async () => {
    const res = await POST(makeRequest({ placeId: "ChIJ_test" }), { params: Promise.resolve({}) });
    expect(res.status).toBe(400);
  });

  it("returns 500 when service.create throws", async () => {
    mockCreate.mockRejectedValue(new Error("DB failure"));
    const res = await POST(makeRequest({ placeId: "ChIJ_err", placeName: "Error Place" }), { params: Promise.resolve({}) });
    const data = await res.json();
  ...
  it("returns 201 without calling generate when generationId is undefined", async () => {
    ...
    const res = await POST(makeRequest({ placeId: "ChIJ_empty", placeName: "Empty" }), { params: Promise.resolve({}) });
    const data = await res.json();
    expect(res.status).toBe(201);
  ...
  it("handles generate failure silently in background", async () => {
    ...
    const res = await POST(makeRequest({ placeId: "ChIJ_bg", placeName: "BG Place" }), { params: Promise.resolve({}) });
    expect(res.status).toBe(201);
