...
/**
 * Format business hours into a brief range string for fine dining footer.
 * e.g. "Mon-Fri 5PM-11PM" or "Tue-Sun 5PM-10PM"
 */
function formatHoursBrief(hours: BusinessHoursMap): string {
  ...
  const openDays = dayOrder.filter(
    (d) => hours[d] && !hours[d].closed
  );
  ...
  const timeRange = `${hours[firstDay].open}-${hours[firstDay].close}`;

  if (openDays.length === 1) {
    return `${dayLabels[firstDay]} ${timeRange}`;
  }

  return `${dayLabels[firstDay]}-${dayLabels[lastDay]} ${timeRange}`;
}
...
export function FooterSection({
  ...
  const fullAddress =
    address && city && state
      ...
      : "";
  const hoursBrief = formatHoursBrief(businessHours);

  const infoParts = [fullAddress, phone, hoursBrief].filter(Boolean);
  ...
  return (
    <footer className="py-12 bg-gray-900">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        {/* Copyright */}
        <p className="text-gray-700 text-xs font-light mt-8">
