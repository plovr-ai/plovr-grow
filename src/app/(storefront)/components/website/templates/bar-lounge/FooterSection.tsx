...
/**
 * Format business hours with emphasis on weekend/late night hours.
 */
function formatHours(hours: BusinessHoursMap): { day: string; time: string }[] {
  ...
  return dayOrder
    ...
    .map((d) => ({
      day: dayLabels[d],
      time: hours[d].closed ? "Closed" : `${hours[d].open} - ${hours[d].close}`,
    }));
}
...
export function FooterSection({
  ...
  const fullAddress =
    address && city && state
      ...
      : "";
  const hoursEntries = formatHours(businessHours);
    ...
    <footer className="py-16 bg-gray-950 text-gray-400">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {/* Brand */}
          <div>
            <h3 className="text-white text-lg font-bold drop-shadow-[0_0_8px_hsl(var(--theme-primary)/0.3)]">
              {name}
            </h3>
            {fullAddress && (
              <p className="mt-4 text-sm">{fullAddress}</p>
            )}
            {phone && (
          ...
          {hoursEntries.length > 0 && (
            <div>
              <h4 className="text-white text-sm font-medium uppercase tracking-wide mb-4">
                Hours
              </h4>
